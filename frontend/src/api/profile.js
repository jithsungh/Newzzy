...
const resetInterests = async (setLogin) => {
  try {
    ...
    return {
      ...
      error:
        error.response?.data?.message ||
        "An error occurred while resetting interests.",
    };
  }
};
...
const updateProfilePicture = async (profile_url, setLogin) => {
  try {
    ...
    const response = await api.put("/profile/updateprofilepicture", {
      profile_url,
    });
...
const setInterests = async (interests, setLogin) => {
  try {
    ...
    return {
      ...
      error:
        error.response?.data?.message ||
        "An error occurred while updating interests.",
    };
  }
};
...
const checkUserInterests = async () => {
  try {
    const response = await api.get("/profile/checkinterests");
    if (response.status === 200) {
      console.log("User interests checked successfully:", response.data);
      const { AccessToken } = response.data;
      localStorage.setItem("AccessToken", AccessToken);
      return { success: true, data: response.data };
    }
    ...
    return {
      ...
      error:
        error.response?.data?.message ||
        "An error occurred while checking interests.",
    };
  }
};
...
const getStreak = async () => {
  try {
    ...
    return {
      ...
      error:
        error.response?.data?.message ||
        "An error occurred while retrieving streak data.",
    };
  }
};
const getRecentActivity = async () => {
  try {
    ...
    return {
      ...
      error:
        error.response?.data?.message ||
        "An error occurred while retrieving recent activity.",
    };
  }
};
export {
